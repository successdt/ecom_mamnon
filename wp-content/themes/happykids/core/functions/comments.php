					...
					<div class="comment-text">
						...
						<?php
							comment_reply_link(array_merge( $args, array(
								'depth' => $depth,
								'reply_text' => '<span class="color_accent">'. multitranslate("Trả lời", "_comments_reply", false) .'</span>',
								'max_depth' => $args['max_depth']
							))); 
						?>
						<?php edit_comment_link( __( '<span class="comment_edit_span">'. multitranslate("Edit", "_comments_edit", false).'</span>' , 'cws_kids' ) ); ?>

						<?php printf(__('%1$s lúc %2$s' , 'cws_kids'), get_comment_date(),  get_comment_time()) ?>
						</div><!--/ comment-meta -->
					</div><!--/ comment-text -->
